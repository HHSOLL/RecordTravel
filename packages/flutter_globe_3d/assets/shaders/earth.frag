...
uniform sampler2D uHighlightTexture; // Index 2: 高亮
uniform sampler2D uBorderTexture;    // Index 3: 国界
...
void main() {
    ...
    if (h > 0.0) {
        ...
        if (t > 0.0) {
            ...
            vec3 borderColor = mix(vec3(0.95, 0.97, 1.0), vec3(0.15, 0.23, 0.35), uDarkTheme);
            finalColor = mix(finalColor, borderColor, borderMask * 0.62);
            float highlightMask = highlightSample.a * uHasHighlightTexture;
            finalColor = mix(finalColor, max(finalColor * (1.0 + highlightMask * 0.45), highlightSample.rgb), highlightMask * 0.52);
