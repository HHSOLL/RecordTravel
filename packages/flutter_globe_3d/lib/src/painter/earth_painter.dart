...
import 'package:flutter_globe_3d/src/models/earth_connection.dart';
...
class EarthShaderPainter extends CustomPainter {
  ...
  final ui.Image? borderTexture;
  final bool hasNightTexture;
  final bool hasHighlightTexture;
  final bool hasBorderTexture;
  ...
  EarthShaderPainter({
    ...
    this.hasHighlightTexture = false,
    this.hasBorderTexture = false,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // [Index 11] 是否有高亮贴图
    shader.setFloat(11, hasHighlightTexture ? 1.0 : 0.0);
    // [Index 12] 是否有国界贴图
    shader.setFloat(12, hasBorderTexture ? 1.0 : 0.0);
    // [Index 13] 当前是否暗色模式
    shader.setFloat(13, isDarkTheme ? 1.0 : 0.0);
    ...
    // [Sampler 2] 国家高亮纹理
    shader.setImageSampler(2, highlightTexture ?? texture);
    // [Sampler 3] 国界纹理
    shader.setImageSampler(3, borderTexture ?? texture);
...
class LinesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var entry in controller.connectionPaths.entries) {
      ...
      final glowPaint = Paint()
        ..style = PaintingStyle.stroke
        ..strokeCap = StrokeCap.round
        ..color = conn.color.withValues(alpha: conn.isDashed ? 0.18 : 0.28)
        ..strokeWidth = conn.width + 2.4
      ...
      if (conn.animatePulse) {
        _drawPulseOnPath(canvas, path, conn, time);
      }
    }
  }
  ...
  void _drawPulseOnPath(
    Canvas canvas,
    Path path,
    EarthConnection connection,
    double time,
  ) {
    final metric = _primaryMetric(path);
    if (metric == null || metric.length <= 0) {
      return;
    }

    final seed = (connection.fromId.hashCode ^ connection.toId.hashCode).abs();
    final progress = ((time * 0.22) + ((seed % 1000) / 1000.0)) % 1.0;
    ...
    final glowPaint = Paint()
      ..color = connection.color.withValues(alpha: 0.36)
    ...
    final shellPaint = Paint()
      ..color = connection.color.withValues(alpha: 0.94)
      ..style = PaintingStyle.fill;

    canvas.drawCircle(center, 8, glowPaint);
    canvas.drawCircle(center, 4.2, shellPaint);
    canvas.drawCircle(center, 1.8, corePaint);
  }

  ui.PathMetric? _primaryMetric(Path path) {
